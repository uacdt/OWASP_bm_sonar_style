...
@WebServlet(value="/weakrand-02/BenchmarkTest01291")
public class BenchmarkTest01291 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String param = request.getParameter("BenchmarkTest01291");
		if (param == null) param = "";
...
"Randomness Test java.security.SecureRandom.nextBytes(byte[]) executed"
    ...
    private class Test {
		...
		if (param != null && param.length() > 1) {
		    StringBuilder sbxyz42950 = new StringBuilder(param);
		    bar = sbxyz42950.replace(param.length()-"Z".length(), param.length(),"Z").toString();
		}
