...
@WebServlet(value="/weakrand-01/BenchmarkTest00585")
public class BenchmarkTest00585 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		while (names.hasMoreElements() && flag) {
			...
			if (values != null) {
				for(int i=0;i<values.length && flag; i++){
					...
					if (value.equals("BenchmarkTest00585")) {
						param = name;
					    flag = false;
					}
				}
		...
		StringBuilder sbxyz49441 = new StringBuilder(param);
		String bar = sbxyz49441.append("_SafeStuff").toString();
