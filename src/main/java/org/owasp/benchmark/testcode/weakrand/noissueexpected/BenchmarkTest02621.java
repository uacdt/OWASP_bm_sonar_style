...
public class BenchmarkTest02621 extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (paramLoc == -1) {
			response.getWriter().println("getQueryString() couldn't find expected parameter '" + "BenchmarkTest02621" + "' in query string.");
			return;
		}
		
		String param = queryString.substring(paramLoc + paramval.length()); // 1st assume "BenchmarkTest02621" param is last parameter in query string.
...
"Weak Randomness Test java.security.SecureRandom.nextInt() executed"
	...
	private static String doSomething(HttpServletRequest request, String param) throws ServletException, IOException {
		...
		if (param != null && param.length() > 1) {
		    StringBuilder sbxyz33448 = new StringBuilder(param);
